...
export function KVTable({ rows, emptyLabel }: { rows: KV[]; emptyLabel: string }) {
  ...
  return (
    <div className="rounded-md border border-border overflow-x-auto text-xs font-mono">
      <div className="grid grid-cols-[110px_1fr] sm:grid-cols-[180px_1fr] min-w-[320px] border-b border-border bg-surface">
      ...
      {rows.map((r, i) => (
        <div
          key={`${r.key}-${i}`}
          className="grid grid-cols-[110px_1fr] sm:grid-cols-[180px_1fr] min-w-[320px] border-b border-border/50 last:border-0"
        >
